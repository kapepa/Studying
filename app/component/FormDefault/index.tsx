...
const FormDefault: FC<FormDefaultInterface> = ({title, inputs, cb, submitText= "Submit"}) => {
  ...
  return <div className={style['form-default']}>
    ...
    <form className={style['form-default__form']} onSubmit={handleSubmit(onSubmit)} role="form">
      {inputs.map(((input: InputType, index: number) => <div key={`${input.name}-${index}`} className={style['form-default__cell']}>
        ...
        <input
          ...
          {...register(input.name, { ...checkPattern(input.name, input.patterns) as RegisterOptions })}
          role="input"
        />
      ...
      <div className={style['form-default__basement']}>
        <button className={style['form-default__submit']} type="submit" role="submit">{submitText}</button>
      </div>
    </form>
  </div>
}
